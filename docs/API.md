...
| `searchEmails` | `view?, relativeDays?, dateFrom?, dateTo?, sender?, keyword?, readStatus?` | Writes `useFilterStore`, navigates to the target list if not already there, fetches a preview, and renders `EmailPreviewCard` inline in chat. Defaults to `inbox` — the system prompt explicitly forbids inferring `sent` just because that's the page currently open. The tool *result* is capped to 8 truncated previews: results live in the chat history and are re-sent on every later message, so an uncapped page would permanently blow Groq's free-tier token budget for the whole thread |
